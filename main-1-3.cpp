...
extern PersonList deepCopyPersonList(PersonList peopleList);    
...
int main() {
    ...
    PersonList deepCopy = deepCopyPersonList(peopleList); // Call deep copy function

     for (int i = 0; i < n; i++) {
        cout << deepCopy.people[i].name << " " << deepCopy.people[i].age << endl; // Checking if copied right
    }
